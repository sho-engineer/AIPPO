...
CHECKS: dict[str, tuple[str, re.Pattern[str]]] = {
    ...
    #
    # ここから下は、上の4つより**あとの段**で見るもの。
    #
    # 上の4つは「1回の指示をどう書くか」で、Lv.3 までで身につく。
    # Lv.4 は**使い分け**（どの基準で比べ、どこを自分で確かめ、何を
    # 渡さないか）、Lv.5 は**組み立て**（何回かに分け、前の結果を次へ
    # 渡し、最後に何を受け取るか）を見る。
    #
    "basis": (
        "比べる基準",
        re.compile(
            r"(基準|観点|軸|費用|価格|金額|コスト|手間|時間|工数|納期|品質"
            r"|実績|メリット|デメリット|長所|短所|比べ|比較|違い|どちら)"
        ),
    ),
    "verify": (
        "自分で確かめるところ",
        re.compile(
            r"(確か|裏取り|裏付け|事実|出典|根拠|元の|原文|自分で|チェック"
            r"|検証|合っている|正しい|誤り|間違い|鵜呑み|うのみ)"
        ),
    ),
    "safety": (
        "渡さない情報",
        re.compile(
            r"(伏せ|置き換え|匿名|仮名|A社|B社|社名|固有名詞|個人名|実名"
            r"|そのまま渡さ|入れない|入れず|省いて|外して|消して|マスキング)"
        ),
    ),
    "steps": (
        "手順に分ける",
        #: `手順書` は除く。**あれは受け取るものの名前**で、分ける話では
        #: ない。除かないと「手順書を作ってください」の1文で、分けた
        #: ことになってしまう（実測で踏んだ）。
        re.compile(
            r"(手順(?!書)|ステップ|段階|順番|順に|分け|まず|はじめに|最初に"
            r"|次に|最後に|1つずつ|一つずつ|何回か|複数回|数回)"
        ),
    ),
    "handoff": (
        "前の結果を次に渡す",
        re.compile(
            r"(前の|その結果|出てきた|受け取った|それをもと|それを使って"
            r"|踏まえて|続けて|次の入力|次へ渡|渡して|引き継)"
        ),
    ),
    "deliverable": (
        "最後に受け取るもの",
        re.compile(
            r"(手順書|一覧|表|箇条書き|リスト|文書|資料|原稿|完成|仕上げ"
            r"|成果|形式|形で|まとめて|出してください|出力)"
        ),
    ),
}
