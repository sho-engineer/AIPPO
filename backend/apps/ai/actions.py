...
REWRITE = Action(
    ...
    # やさしさの線を、数で引いてある理由。
    #
    # 前は「むずかしい言葉はかみくだく」「言いかえられない言葉は元の言葉を
    # 残して（〜のこと）を添える」と書いてあった。後者は意味のねじれを
    # 止めるために置いた逃げ道だが、**専門語しか出てこない文章では、
    # 全部がその逃げ道に当てはまる**。実機の Day1 は、用語がそのまま
    # 並んでカッコ書きが付いただけの文章を返していて、読む側の負担は
    # ほとんど減っていなかった。
    #
    # だから逃げ道は残したまま、2つ足してある。**順番**（普通の言葉が主、
    # 元の用語は従）と、**数**（そのまま使ってよいのは多くて2つ）。
    # 数で言わないと「必要なだけ」と読まれ、必要の判断はモデルがする。
    # 2つを超えて残ったものは `quality.plain_enough` が拾う。
...
- 指定が無いものは、勝手に決めつけない。とくに**短くしろとは言われていない**
  ——長さの指定が無いときは、元の内容が落ちない長さで書く。
  ただし、説明を足して長くはしない
- むずかしい言葉はかみくだく。どこまでかみくだくかは、下の「合格の線」で決める
- 元の内容の意味は変えない
...
**その分野を何も知らない人が、一度読んで意味をつかめること。**

これが合格の線。「読めば分かる」ではなく「**一度読んで分かる**」。
読み返さないと分からない文は、まだ書き直しが終わっていない。
...
- **普通の言葉を主にし、元の用語は従にする。**
  意味を保ったまま言いかえられない言葉でも、元の言葉を先に置かない。
  まず普通の言葉で書き、そのうしろに「（元の言葉）」と添える。
  NG: 「ステークホルダー（関係者のこと）と合意形成を図る」
  OK: 「関わる人たちに話を通して、納得してもらう」
- **元の文章の用語を、そのままの形で使ってよいのは多くて2つまで。**
  数で縛るのは、「必要なだけ残してよい」と読まれると、
  専門語だらけの文章では**全部が必要**になってしまうため。
  3つ目からは、その語を使わずに何をしているかだけを書く
- 仕組みの内部は書かない。計算の手順、式、部品の名前、処理の順番は、
  読む人が知りたいことではない。知りたいのは「**結局それは何をして
  いるのか**」だけ。中身は省いて、はたらきだけを書く
- 1文は短くする。40字くらいを目安に、ひとつの文にひとつのことだけ書く
- 説明を足して長くしない。長くするのではなく、中身を省いて分かりやすくする
...
SUMMARIZE = Action(
    ...
    system_prompt=BASE_RULES
    + """
やること: 与えられた文章を、指定された読む人・目的・形式・長さでまとめる。

**元の文章に書かれていないことは足さない。**

- 数字は書き換えない。割合・人数・期間は、元の文章のとおりに書く
- 結論や推奨を勝手に作らない。元が「案が出ている」なら「案が出ている」と書く
- 分からないことは書かない

**読む人と目的が指定されているときは、そこに必要な情報を優先して残す。**

- 判断のために読む人には、判断の根拠になる数字・懸念・次の行動を残す
- 全部を平等に削らない。要約は短くすることではなく、**残すものを選ぶこと**

**指定された出力形式を、そのとおりに守る。**

- 「3つの箇条書きで」と言われたら、**項目はちょうど3つ**にする
- 箇条書きに見出し・前置き・結論の行を足さない

**1文目から、まとめた文章そのものを書きはじめること。**

- 「承知しました」「以下のようにまとめます」のような作業の宣言を頭に置かない
- 「以下が要約です：」のような見出しも書かない
- 頼まれているのはまとめであって、作業の報告ではない

まとめた結果だけを result に入れる。前置きも、あとがきも付けない。
""",
    schema=TEXT_SCHEMA,
    fields=(
        ActionField("original_text", "元の文章", max_length=5000),
        # 読む人・目的・形式・長さは**任意**。
        #
        # Day2 の最初の1回は「この文章を短くまとめて」だけを送る
        # ——形も目的も、そのあとで足して違いを見るのが教材のねらい。
        # 必須のままだと、その1回目が 400 で返る（Day1 の `rewrite` で
        # 同じことが起きて、同じように緩めてある）。
        #
        # 他の教材（extract_needed_info など）は条件を渡すので、
        # 緩めても送られる内容は変わらない。空のときは依頼文から行ごと
        # 落ちる（`_line`）ので、「指定なし」が文章として混ざることもない。
        ActionField("audience", "読む人", required=False),
        ActionField("purpose", "まとめる目的", required=False),
        ActionField("format", "出力形式", required=False),
        ActionField("length", "長さ", required=False),
        ActionField("instruction", "追加の条件", required=False),
    ),
    body_field="original_text",
