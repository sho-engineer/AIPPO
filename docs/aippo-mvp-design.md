...
> **追記（ゲストの期限を90日から7日へ）**: Cookie の寿命は **7日** にした
> （`LEARNER_KEY_MAX_AGE`）。合わせて、**取っておくもの**——あとで見る印・
> 自分のプロンプト帳・修了証——は登録した人だけのものにした
> （`apps/accounts/scope.py` の `can_keep()` と `frontend/src/course/keeping.ts`）。
>
> 匿名で**始められる**性質と、学ぶこと自体が無料である点は変えていない。
> 変えたのは「登録しないまま、いつまでも溜められる」ところ。90日の鍵で
> 溜めさせると、取っておいたつもりのものが本人からも取り出せなくなる日が
> 黙って来る。取っておけると見せてから消すより、取っておくには登録が要ると
> 先に言うほうを選んだ。ゲストの記録の保持も 180日 → **30日**。
