...
| 種類 | 件数 | 実行方法 |
| --- | --- | --- |
| バックエンド（pytest） | 112 | `cd backend && pytest` |
| フロントエンド（Vitest） | 86 | `cd frontend && npm test` |
| E2E（Playwright / APIスタブ） | 11 × 2画面 | `npx playwright test e2e/lesson.spec.ts` |
| アクセシビリティ（axe / WCAG 2.1 A・AA） | 6 × 2画面 | `npx playwright test e2e/a11y.spec.ts` |
