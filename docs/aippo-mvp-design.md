...
| **5. ログ取得** | ✅ **完了**（学習イベント・セッション再開）／アンケートは**画面が無い**（下記） |
| **6. E2Eテスト** | ⚠ **要作り直し**（成果物ファーストの流れに追随できておらず失敗する。CI では `a11y` と `smoke` を動かしている） |

> **アンケートの画面が落ちている。** 成果物ファーストへ作り直したときに
> 完了画面からアンケートが外れた。API（`POST /api/lessons/{id}/survey/`）と
> `Survey` モデルは残っているが、答えを入れる画面が無い。
> フェーズ2→3 の判定条件のひとつ「有料テストの申込率」がこれで測れないので、
> クローズドベータを始める前に、戻すか別の方法にするかを決める
> （`docs/roadmap.md`）。
...
| `Survey` | ⚠ | 完了時アンケート（モデルと API はあるが、**入力画面が無い**） |
