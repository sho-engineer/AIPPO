    ...
    */
    ...
    /*
      ...
      題までの横の余白を測るとこうなっている。
          ...
          章①左 7.8% 右 6.1% ／ 章②左 3.9% 右 4.7%
          章③左10.1% 右 9.0% ／ 章④左 6.3% 右 7.7%

      いちばん狭いのは章②の左 3.9%。細長い持ち方（402×874）では
      左右を 7.0% 切るので、**章②は題の端が欠ける**。
  ...
  test("「つづける」が、絵より前に出ない", async ({ page }) => {
    ...
    */
    ...
    /*
      ...
      絵の側が、下に 18〜29% の雲を空けてくれている（章②が 29.1% で
      いちばん広く、章①が 18.2% でいちばん狭い）。ボタンはその中に
      収まっていればよい。

      だからボタンの上端が絵の何%に当たるかを見る——`object-cover`
      で切ってあるので、画面の座標ではなく**絵そのものの座標**に
      直してから測る。
    */
