...
describe("つないだ1行", () => {
  ...
  it("Day1 は、その日いちばん最初におぼえたことを言う", () => {
    /*
      到達点の1本目そのまま（`outcomes[0]` ＋「ようになりました」）。

      Day1 の到達点は、章の並びと同じ順に3本ある——頼む・読む人・
      伝え方。その1本目なので、祝いの1行は「AIにしてほしいことを
      伝えられるようになりました」になる。

      **前は「むずかしい文章を、意味を変えずに…」だった。** あれは
      到達点が1本しか無かったころの文で、いまはその内容が完了画面の
      見出し（「文章を相手に合わせて変えられるようになりました」）と
      3本の箇条書きに分かれている。
    */
    const day1 = lessons.find((lesson) => lesson.id === "rewrite_text");
    expect(dayOutcomeLine(day1!)).toBe(
      "AIにしてほしいことを伝えられるようになりました",
    );
  });
});
