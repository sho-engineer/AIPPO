...
/**
 * 無料で使える分を使い切ったときの言葉（`components/course/LessonPaused.tsx`）。
 *
 * 出さない言葉
 * ------------
 * 「Quota Exceeded」「Credit 切れ」「API 上限」「トークン上限」。
 * どれも**こちら側の都合の名前**で、押した人がしたこととは関係が無い。
 * 読んでも次に何をすればよいか分からず、「壊れた」としか読めない。
 *
 * 数字で急かさない
 * ----------------
 * 「残り0回」「あと3時間で回復」も出さない。急がせて登録させても、
 * 次に開く理由にはならない。
 */
export const PAUSED_COPY = {
  title: "今日の練習はここまで！",
  doneTitle: "今日できるようになったこと",
  /*
    登録すると何が変わるかを、**その人にとっての言葉**で1行。
    「クレジットが付与されます」ではなく「続きをそのまま進められます」。
  */
  registerLead: "登録すると、今日の続きをこのまま進められます。無料です。",
  registerNow: "今すぐ続きを無料ではじめる",
  waitTomorrow: "明日また続ける",
} as const;
...
export const AUTH_COPY = {
  ...
  */
  ...
  /*
    ...
    「どれで登録するか」を先に選ばせる。押した道に要るものだけを、
    そのあとで聞く。前は3つの道の入力が1枚に混ざっていて、
    パスキーで登録する人にもパスワード欄が見えていた。
  */
  continueWithPasskey: "パスキーで続ける",
  /*
    パスキーを一番上に置くなら、それが何かを押す前に言う。
    知らない言葉が一番上にあると、知っている言葉のほうへ逃げる。

    仕組み（公開鍵暗号）の話はしない。**その人にとって何が変わるか**
    だけを書く。
  */
  passkeyLead: "指紋・顔・画面ロックで入れます。パスワードを覚える必要がありません。",
  continueWithEmail: "メールで続ける",
  methodDivider: "または",
  /** 2枚目。押した道に要るものだけを聞く。 */
  passwordStepLead: "このメールアドレスで登録します。パスワードを決めてください。",
  passkeyStepLead:
    "パスキーで登録します。パスワードは決めなくてかまいません。",
  changeEmail: "変更する",
  back: "もどる",
  ...
  /*
    ...
    **どの登録方式でも同じにする。** 前は Google だけ「押したら同意」で、
    パスキーとパスワードにはチェック欄があった。同じ登録なのに
    求められるものが違い、しかもチェック欄は Google のボタンを
    素通りしていた——つまり厳しく見えて、実際には守っていない。

    いまはどの道でも、押す前にこの一文が見えるところにある。
    規約とポリシーは、この画面の中で読める（外へ飛ばすと入力が消える）。
  */
  consentNotice: "続けると、利用規約とプライバシーポリシーに同意したことになります。",
  ...
  /*
    ...
    一般向けの学習サービスで全員に求めると、そこで止まる人のほうが多い。
  */
  mfaTitle: "確認コードを入れてください",
  mfaLead:
    "認証アプリに出ている6桁の数字を入れてください。予備の合言葉でも入れます。",
  mfaCode: "確認コード",
  mfaSubmit: "確認する",
...
export const SOCIAL_COPY = {
  ...
  /*
    ...
    「送信中」ではなく「移動中」だと分かる言い方にする。ここで
    止まっているように見えると、押し直して往復が2本走る。
  */
  leavingTo: (label: string) => `${label} に接続しています…`,
