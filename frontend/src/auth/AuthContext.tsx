    ...
    () => ({
      ...
      markDiagnosisNudgeSeen() {
        /*
          ...
          では消える**。組み替えが起きれば数え直しになる。ここの
          `ref` は `AuthProvider` のもので、アプリが開いているあいだ
          作り直されないので、呼ぶ側がいくつ増えても1回で済む。

          読み込み直せば当然また送る。それは正しい——別の立ち上がりで
          案内を出したなら、その回のぶんを残す。
          ...
          描画のときで、呼んだその場では値が返ってこない。同じ回に
          2回呼ばれると、どちらも「まだ」と読んで2回送る。
