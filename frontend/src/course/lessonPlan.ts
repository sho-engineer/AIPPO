...
export interface LessonPlan {
   ...
   * 見比べたあとに、持ち帰る一言。
   *
   * 「変わったところ」の✓は、**この1回に何が起きたか**しか言っていない
   * （むずかしい言葉を減らした、一文を短くした…）。それは結果であって、
   * 次に自分で頼むときには使えない。
   *
   * ここで1行だけ、**何を足したから変わったのか**を言う。同じ手は
   * 別の文章でも効くので、これが持ち帰れる唯一のもの。
   *
   * 短く保つ。ここは見比べる画面の**いちばん下**で、長く書くと
   * 「変わったところを見る」が画面の外へ出る。いまの一言（22字）は
   * 幅 390 で2行・64px。低い持ち方（402×660）でもちょうど収まるが、
   * **余っているわけではない**ので、伸ばすなら測り直すこと
   * （`e2e/stepFits.spec.ts`）。`tests/lessonPlan.test.ts` が字数を見張る。
   */
  takeaway: string;
  /**
   * 図の下でポーが言う一言。
   ...
   * 3手をまとめて指して答える（前は「この2つを足してみよう！」と
   * 真ん中の＋だけを指していたが、図が3手そろった形になったので、
   * 上下の箱を指さない言い方だと片手落ちになる）。
   */
  poLine: string;
...
export const LESSON_PLANS: Record<string, LessonPlan> = {
  rewrite_text: {
    ...
    /*
      「やさしくなった」とは書かない。**それは結果で、手ではない。**
      次に別の文章で困ったとき、思い出して使えるのは「誰に、を足す」の
      ほう。主語は学習者（AIではなく）にする。
    */
