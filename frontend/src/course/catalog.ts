...
const LESSON_1: Lesson = {
  ...
  */
  ...
  /*
    ...
    専門用語を訳しただけの文にしない
    --------------------------------
    最初の例文は「文章の中にある言葉同士の関係を調べながら、どの言葉に
    注目するべきかを判断します」で、元の用語を1つずつ日本語に置き換えた
    形をしていた。対応はたどれるが、**読んでも分かった気がしない**。

    次に「AIは文章を読むとき、すべての言葉を同じように見るわけでは
    ありません」から始まる3文にした。読み口はやさしくなったが、**元の
    文が言っていたことが落ちた**。書き出しが打ち消しなので、Before の
    どこがここになったのかもたどれない。完成イメージは1文目しか出さない
    ので、画面には**その打ち消し1文だけ**が残っていた。
    ...
    Query・Key・Softmax・Value といった内部の計算はここでは出さない。
  ...
  */
  ...
  /*
    ...
    前は「誰向けかを伝える」「言い方を伝える」という**動作の説明**で、
    本編のスタンプ（プロンプト／ターゲット指定／トーン指定）と食い違って
    いた。始める前に見た名前と、取ったときの名前が違うと、同じものだと
    気づけない。数も2つと3つでずれていた。
  */
  learnedSkills: ["プロンプト", "ターゲット指定", "トーン指定"],
  ...
  steps: buildLessonFlow({
    ...
    */
    ...
    /*
      1行だけ添える。**何を選ぶのかではなく、何を基準に選ぶのか**を言う。

      前は空にしていた（「何が起きるかは下のボタンが言っている」）。
      ところが3つとも「分かりやすくする」方向で、字面だけでは
      どれを選んでも同じに見える。決め手は目的のほうにある。
    */
    quickInstruction: "目的に合わせて、書き直しの方向を選びましょう。",
    quickKey: "instruction",
    /*
      1行の補足を添える。札の名前だけだと、「かみくだいて説明する」と
      「要点から先に伝える」の違いが読み取れない。

      補足に「短く」は書かない。**Day1 は「難しい → 分かる」で、
      「長い → 短い」は Day2 の要約が持つ**（`tests/lessonPlan.test.ts`
      が画面に出る言葉を見張っている）。支給デザインの
      「長い文章を、短く・シンプルにする」は、そのまま入れると
      Day2 と役が被る。
    */
    quickOptions: [
      {
        value: "専門用語を減らす",
        label: "専門用語を減らす",
        note: "むずかしい言葉を、やさしい言葉に言いかえる",
        icon: "document",
      },
      {
        value: "かみくだいて説明する",
        label: "かみくだいて説明する",
        note: "ひとつずつ、かんたんな言い方に直す",
        icon: "chat",
      },
      {
        value: "要点から先に伝える",
