...
const LESSON_1: Lesson = {
  ...
  */
  ...
  /*
    ...
    元の文と1対1で対応させる
    ------------------------
    前の例文は、元の文には無い喩え（「それ」が何を指しているか）を
    足していた。読めば分かりやすいが、**元の文のどこが変わったのかを
    たどれない**——AIが別の説明を書き下ろしたように見える。それだと
    この回のねらい（意味は同じまま、言い方だけ変わる）が伝わらない。

    いまの例文は、元の3つの用語がそれぞれどこへ化けたかを追える。
    対応そのものは `course/lessonPlan.ts` が表として持っていて、
    ずれると `tests/lessonPlan.test.ts` が落ちる。
  */
  afterExample:
    "TransformerというAIは、文章の中にある言葉同士の関係を調べながら、どの言葉に注目するべきかを判断します。さらに、複数の視点から同時に文章を見ることで、言葉同士のつながりをより詳しく理解できます。",
