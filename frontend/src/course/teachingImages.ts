...
export interface TeachingImageEntry {
   ...
   * いまは画面が引いていない
   * ------------------------
   * レッスンの開始画面から `所要時間` の札そのものを外したので
   * （入口を2つに絞る作り直し）、この値で出し分けている場所は無い。
   * 残してあるのは**絵が何を言っているかの控え**として。
   * `tests/teachingImageFacts.test.ts` が、絵の中の数字と教材データが
   * 食い違っていないかを、いまもこれを手がかりに見張っている。
   *
   * 時間はポーの一言が言う（「今日やること」の一枚）。絵を差し替えて
   * 時間が消えたら、ここも false（または省略）に戻すこと。
   */
...
const BY_LESSON: Record<string, Record<string, TeachingImageEntry>> = {
  ...
  */
  ...
  /*
    Day1「文章を分かりやすくする」には、解説の絵と比べる図を置かない。
    ...
    **どれも画面の中の比較と同じことを言っていた**から。
    ...
    残したのは全体図（`outcome_preview`）だけ。**ただしレッスンの中
