...
/**
 * ブラウザが返した断り方を、直せる言葉にする。
 *
 * 前は理由を全部捨てて「パスキーを使えませんでした」の一文にしていた。
 * 押した人には何も分からないし、こちらも報告から原因を絞れない。
 * 実際、**設定の取り違えで必ず失敗する状態**が、この一文の裏に
 * 隠れられてしまう（下の SecurityError がまさにそれ）。
 *
 * いちばん多い2つだけ言い分ける。細かく分けるほど当たらなくなるので、
 * 残りは元の名前を添えて返す——こちらが受け取る報告に手がかりが残る。
 */
export function browserReason(error: unknown, verb = "使え"): string {
  const name = error instanceof Error ? error.name : "";

  if (name === "SecurityError") {
    /*
      いま開いているアドレスと、サーバーが署名に混ぜるドメイン
      （backend の PASSKEY_RP_ID）が食い違っている。配置のたびに
      起きうるうえ、症状は「なぜか作れない」だけになる。
    */
    return "この画面のアドレスでは、パスキーを作れません。設定が合っていない可能性があります。";
  }
  if (name === "InvalidStateError") {
    // この端末では作成済み。作り直しではなくログインへ促す
    return "この端末には、すでにパスキーがあります。作り直さずに、そのままお使いください。";
  }
  return name
    ? `パスキーを${verb}ませんでした（${name}）。もう一度お試しください。`
    : `パスキーを${verb}ませんでした。もう一度お試しください。`;
}
