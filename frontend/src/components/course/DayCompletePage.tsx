 ...
 * 「次の段まで、あと何つ」も置かない
 * ----------------------------------
 * 一度は完了画面に置いて、出ないまま戻した。ここへ移せば出せる
 * （記録は直前に確定している）が、**置かないと決めた。**理由は3つ。
 *
 *   1. 残り数は、レッスンの開始画面（`LessonSkillLine`）と学習マップが
 *      既に出している。ここで三度目を言う
 *   2. 記録の送信は待たずに画面が変わる（`finalizeCompletion` の
 *      `void completeLesson`）。ここで問い合わせると追い越して、
 *      **祝いの画面に古い数**が出る回がある
 *   3. 上の5つに6つ目を足すことになる。1画面に収まることは
 *      E2E が実寸で見張っている（727px / 720px）
 *
 * 出すなら、サーバーの返事に段を載せてから。そのときは 3 も測り直す。
 *
 * スマホでは縦にスクロールさせない
 * --------------------------------
...
export interface DayCompletePageProps {
   ...
   */
  nextDay?: number;
  /** いちばん押してほしい行き先。無い回はホームに戻るだけになる。 */
  primary?: { label: string; onClick: () => void };
  /** コースの道のりへ戻る。**必ず在る**（行き止まりにしない）。 */
  /**
   * 締めくくりの出口。**ホームへ。**
   *
   * ここは長く「コースに戻る」でコースの中身へ渡していた。けれど
   * 1本終えた直後に見たいのは、**その1本が数に反映されたところ**で、
   * それを出しているのはホーム（「これまでの記録 n / 4」と
   * 「今週の学習」）。コースの中身へ戻すと、いま終えた行に印が付く
   * だけで、終えた実感が数に乗らない。
   *
