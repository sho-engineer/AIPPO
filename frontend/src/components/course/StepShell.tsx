  ...
  return (
    ...
    */
    ...
    >
      ...
      */}
      {/*
        ここから下の3つ——進み具合・見出し・その回の中身——は、
        **1つの送り枠の中**に入れる。

        なぜ枠の外に置かないか
        ----------------------
        前は進み具合と見出しを枠の外の「動かないもの」にしていた。
        ふだんは何も問題が無い。困るのは**画面が急に低くなるとき**
        ——ソフトキーボードである。

        320×568 で入力欄に触れると、見える高さは 352px になる。柱は
        308px。そこに 進み具合35 ＋ たたんだ答え22 ＋ 見出し138 ＋
        下の帯155 ＝ 350px が、**どれも縮まない**まま積まれていたので、
        下の帯が 50px はみ出し、「次へ」が画面の外にあった（実測）。

        隠して縮めることもできるが、そのとき消えるのは
        「いま何を聞かれているか」そのもの。**隠さずに、送れるほうへ
        移す**。読みたい人は上へ戻せばよく、書いている最中に要るもの
        ——入力欄と「次へ」——は必ず画面に残る。

        送り枠は**ここ1つだけ**。入れ子にはしない（要件 §4）。
      */}
      <div
      ...
      <div className="mt-2 shrink-0 [@media(min-height:700px)]:mt-4">
        <PoHero
          ...
          */
          ...
          burst={poBurst}
          /*
            低い持ち方ではポーを引っ込める。**キーボードのため。**

            見える高さが 560px を切るのは、実機ではほぼキーボードが
            出ているとき（320×568 で 352px、375×667 で 451px）。
            そこでポーの分（約110px）を返さないと、下の「次へ」が
            画面の外へ出る。書く画面で入力欄とボタンが同時に見える
            ことは、譲れない条件（要件 §6.11）。
          */
          hideWhenShort
        />
      ...
      */}
        {/*
          ...
          **残りの高さに収まるまで縮む**——AIの結果や教材の絵のように
          ...
          送るのは1つ外側（`step-stage`）。ふだんの回はそこでも送らずに
          収まる（`e2e/stepFits.spec.ts` が実寸で見張る）。
        */}
        <div
          /*
            低い持ち方では、上の余白を詰める。
