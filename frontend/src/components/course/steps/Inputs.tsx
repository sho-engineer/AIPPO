...
export function TextStep({
  ...
  return (
    ...
    */
    /*
      縦の flex。ただし**自分の中身より縮まない**（`min-h-fit`）。

      `min-h-0 flex-1` だけにしていたころ、320×568 では残りの高さが
      足りず、この箱が中身より 22px 低く潰れていた。中の行
      （文字数・「ヒントを見る」）は縮まないので、そのぶんが箱の外へ
      描かれ、**すぐ下の安全の一言に重なって「ヒントを見る」が押せない**
      状態になっていた（実測・`e2e/_audit.spec.ts`）。

      縮まないと決めれば、入り切らない日は枠のほうが送れる
      （`step-stage`）。重なりは起きない。
    */
