 ...
 *
 * いまの置き場は結果の1枚目（`DiagnosisResult` の読み取り）。5問に
 * 答え終わった直後に、**答えから出した段そのもの**を左から順に
 * 描く。診断らしさは待ち時間ではなくここで出す。
 *
              ...
              return (
                <span
                  ...
                  /*
                    ...
                    最後の1本まででおよそ 0.8 秒。**読み取っている手応え**
                    はここで出す。足止めはしない——下のボタンは最初から
                    押せて、押せばその場で次へ行く。動きを減らす設定では
                    `transition` ごと一瞬で終わる（`index.css`）。
                  */
                  style={{
                    opacity: drawn || !on ? 1 : 0,
                    transform: drawn || !on ? "none" : "scaleX(0.2)",
                    transformOrigin: "left",
                    transitionDelay: `${row * 110 + step * 55}ms`,
                  }}
                />
              );
            })}
