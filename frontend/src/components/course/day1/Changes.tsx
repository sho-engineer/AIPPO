...
export function Changes({
  ...
  return (
    /*
      ...
      縮ませない理由
      --------------
      中には縮まないもの（足した条件・気づきの1行・「全文を見る」）が
      3つある。縮む箱にしていたころは、そこまで押し潰されて**中身が
      箱の外へはみ出し、下の注意書きが「全文を見る」の上へ重なって**
      いた（320×568 で実測。押す場所が 23px 隠れていた）。

      伸ばさない理由
      --------------
      `flex-1` で残りの高さを吸わせていたころ、中身が短い日はその差が
      まるごと**空白**になった。実機の 393×727 で、「全文を見る」と
      下の注意書きのあいだに 176px（実測）。読むものと読むものの
      あいだに空白が入ると、下の行は「別の画面のもの」に見える。

      余りは下の帯との間に残るだけにする。入り切らないときは画面の
      側が送れる（`step-stage`）。
    */
    <div className="flex shrink-0 flex-col" data-testid="day1-changes">
      {/*
        何を変えたか。**今回足したものだけ。**
      ...
      */}
      {/*
        ...
        ここは `flex-1` で、残りの高さを全部吸っていた。中身（1〜2組、
        または返ってきた文章の頭）が短い日は、その差がまるごと
        **空白**になる——実機の 393×659 で 138px、393×727 で 186px、
        結果の箱と下の✓のあいだにぽっかり空いていた（実測）。
        「読み終えたのに、次が遠い」画面になる。

        中身の高さのまま置く。余りは下の帯との間に残るだけで、
        読むものと読むもののあいだには入らない。
      */}
