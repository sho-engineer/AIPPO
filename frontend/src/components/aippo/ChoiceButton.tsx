...
export function ChoiceButton({
  ...
  */
  /*
    ひとこと説明の付く札は、**積まずに横へ組む。**

    積む形は 6.5rem（104px）を確保する。そこへ説明の1行が足されると
    126px になり、7つ並べた診断の最後の問いは 402×660 の持ち方で
    143px はみ出していた（実測）。

    説明が付くのは「文章」「要約」のような**短い題**の札で、
    積んで幅いっぱいを使う理由が無い。絵を左、題と説明を右に置けば
    70px 前後に収まり、7つでも送らずに見える。
  */
  const stacked = tall && Boolean(icon) && !description;
