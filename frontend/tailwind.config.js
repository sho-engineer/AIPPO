    ...
    extend: {
      ...
      /*
        ...
        大きさは支給された6枚から測った。
          ...
          badge  8px … 小さな要素（印、入力欄、小ボタン、札）
          card  14px … 一覧の行、区切られた面
          panel 18px … 独立した操作単位（ダイアログ、主要な面）
      ...
      */
      ...
      /*
        ...
        rounded-lg / -xl / -2xl をそのまま使うと、書く人ごとに一段ずつ
        違う値が入り、画面をまたぐと同じ役割の部品が違う丸みで並ぶ。
        「なんとなくAIが作ったUI」の正体は、たいていこれ。

          badge  8px  … 小さい印・タグ・字だけのボタン。中身より小さい丸み
          card  14px  … 面。囲って1かたまりだと示すところ
          cta   16px  … 押す先が1つだけの大きなボタン
          panel 18px  … 面の中の面ではなく、**画面を占める**まとまり

        丸（rounded-full）は意味のある場所にだけ残す——進み具合の輪、
        順番を表す点、似顔絵。押せる四角を丸くしない。
